...
MODEL_FEATURES_EN = [
    ...
    'TP', 'ALB', 'TBIL', 'DBIL', 'ALT', 'AST', 'FBG', 'HbA1c', 'UAER', 'duration of HT'
]

# 数值型特征
NUMERIC_FEATURES = [
    'Age', 'Course', 'BMI', 'WHR', 'SBP', 'DBP', 'BUN', 'Scr', 'UA', 'TP',
    'ALB', 'TBIL', 'DBIL', 'ALT', 'AST', 'FBG', 'HbA1c', 'UAER', 'duration of HT'
]
...
def create_input_field_chinese(feature_name):
    ...
    if feature_name == '性别':
        ...
        return selected[1]  # 修正：返回元组的第二个元素
...
def load_model_and_preprocessors():
    ...
    try:
        ...
        # 尝试加载imputers
        try:
            median_imputer = joblib.load('final_results/median_imputer_fixed.pkl')
            mode_imputer = joblib.load('final_results/mode_imputer_fixed.pkl')
        except:
        ...
        try:
            ...
            # 过滤selected_features，仅保留模型需要的特征
            selected_features = [f for f in selected_features if f in MODEL_FEATURES_EN]
    ...
    try:
        ...
        for feature in MODEL_FEATURES_EN:
            if feature not in english_input:
                if feature == 'Sex':
                    english_input[feature] = 2  # 默认女性
                elif feature == 'Age':
                    english_input[feature] = 50
                elif feature == 'BMI':
                    english_input[feature] = 24.0
                elif feature in CATEGORICAL_FEATURES:
                    english_input[feature] = CATEGORY_MAPPINGS[feature]['default']
                else:
                    english_input[feature] = 0.0

        # 转换为DataFrame
        input_df = pd.DataFrame([english_input])

        # 使用批量预处理函数处理单患者数据
        processed_data, _ = preprocess_batch_data(input_df, feature_info)
            ...
            if submitted:
                with st.spinner("分析数据中..."):
                    ...
                    if prob is not None:
                        ...
                        ax.barh([0], [prob * 100], color=risk_color, alpha=0.7)
                        ax.set_xlim(0, 100)

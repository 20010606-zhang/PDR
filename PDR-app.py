...
def load_model_and_preprocessors():
    try:
        model = joblib.load('lightgbm_pdr_model.pkl')
    ...
    with col1:
        ...
        with st.form("prediction_form"):
            ...
            with col1_1:
                ...
                smoking = st.selectbox("Smoking History", options=[("No", 0), ("Yes", 1)], format_func=lambda x: x[0])[1]
            
            with col1_2:
                drinking = st.selectbox("Drinking History", options=[("No", 0), ("Yes", 1)], format_func=lambda x: x[0])[1]
                course = st.number_input("Diabetes Duration (years)", min_value=0.0, max_value=50.0, value=5.0, step=0.5)
            ...
            with col1_3:
                ...
                ht = st.selectbox("Hypertension History", options=[("No", 0), ("Yes", 1)], format_func=lambda x: x[0])[1]
                ht_duration = st.number_input("Hypertension Duration (years)", min_value=0.0, max_value=50.0, value=0.0, step=0.5)
            ...
            with col2_1:
                sbp = st.number_input("Systolic Blood Pressure (mmHg)", min_value=60.0, max_value=250.0, value=120.0, step=1.0)
                dbp = st.number_input("Diastolic Blood Pressure (mmHg)", min_value=40.0, max_value=150.0, value=80.0, step=1.0)
                bun = st.number_input("Blood Urea Nitrogen (BUN, mmol/L)", min_value=1.0, max_value=30.0, value=5.0, step=0.1)
                scr = st.number_input("Serum Creatinine (Scr, μmol/L)", min_value=20.0, max_value=500.0, value=70.0, step=1.0)
            ...
            with col2_2:
                ...
                tbil = st.number_input("Total Bilirubin (TBIL, μmol/L)", min_value=1.0, max_value=100.0, value=12.0, step=0.1)
                dbil = st.number_input("Direct Bilirubin (DBIL, μmol/L)", min_value=0.0, max_value=50.0, value=4.0, step=0.1)
            ...
            with col3_1:
                ...
                fbg = st.number_input("Fasting Blood Glucose (FBG, mmol/L)", min_value=3.0, max_value=30.0, value=6.5, step=0.1)
            
            with col3_2:
                hba1c = st.number_input("Glycated Hemoglobin (HbA1c, %)", min_value=4.0, max_value=15.0, value=6.5, step=0.1)
                uaer = st.number_input("Urinary Albumin Excretion Rate (UAER, μg/min)", min_value=0.0, max_value=500.0, value=20.0, step=1.0)

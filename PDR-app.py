...
# 强制安装适配Python 3.13的LightGBM
def install_lightgbm_for_python313():
    try:
        import lightgbm
        st.sidebar.success("✅ LightGBM已安装")
        return True
    except ImportError:
        st.warning("正在安装适配Python 3.13的LightGBM...")

        # 方法1：安装预编译版本
        try:
            subprocess.check_call([
                sys.executable, "-m", "pip", "install",
                "lightgbm==4.4.0", "--no-cache-dir", "--force-reinstall"
            ])
            import lightgbm
            st.sidebar.success("✅ LightGBM安装成功")
            return True
        except Exception as e1:
            st.error(f"预编译版本安装失败：{e1}")

            # 方法2：从源码编译安装
            try:
                subprocess.check_call([
                    sys.executable, "-m", "pip", "install",
                    "git+https://github.com/microsoft/LightGBM.git@master#egg=lightgbm",
                    "--no-cache-dir"
                ])
                import lightgbm
                st.sidebar.success("✅ LightGBM源码编译安装成功")
                return True
            except Exception as e2:
                st.error(f"源码编译安装失败：{e2}")
                return False


# 执行安装
lightgbm_installed = install_lightgbm_for_python313()
